...
> [!NOTE]
> Alpha Vantage recently changed their API endpoint for their free tier, which previously returned the full 20+ years of historical data, by limiting it to the last 100 data points.
...
Below are example CSV files that should be referenced in `appsettings.json` against `SymbolsFullPath`.  Delimited values are: Stock symbol, Excel column name, Share purchase price.

> [!IMPORTANT]
> Stock symbols can differ between Marketstack and Alpha Vantage.
