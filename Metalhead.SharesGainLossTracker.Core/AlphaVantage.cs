...
public class AlphaVantage(ILogger<AlphaVantage> logger, IProgress<ProgressLog> progress) : IStock
{
    async Task<List<FlattenedStock>> IStock.GetStocksDataAsync(HttpResponseMessage[] httpResponseMessages, bool endpointReturnsAdjustedClose)
    {
            ...
            if (item.IsSuccessStatusCode)
            {
                ...
                else
                {
                    if (stock is not null
                        && !string.IsNullOrWhiteSpace(stock.Note)
                        && stock.Note.EndsWith("if you would like to target a higher API call frequency."))
                        hadRateLimitError = true;
                    else if (stock is not null
                        && !string.IsNullOrWhiteSpace(stock.Information)
                        && stock.Information.EndsWith("to instantly remove all daily rate limits."))
                        hadDailyLimitError = true;
                    else if (stock is not null
                        && !string.IsNullOrWhiteSpace(stock.Information)
                        && stock.Information.Contains("This is a premium endpoint"))
                        hadEndpointAccessRestrictedError = true;
                    else if (stock is not null
                        && !string.IsNullOrWhiteSpace(stock.ErrorMessage)
                        && stock.ErrorMessage.StartsWith("Invalid API call."))
        ...
        {
            logger.LogError("Daily API calls limit reached error from stocks API.  Plans with a higher limit may be available.");
        ...
        {
            logger.LogError("Error deserializing data from stocks API.");
